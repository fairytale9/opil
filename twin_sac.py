...
class SAC(object):
  ...
  def fit_actor(self, states, target_entropy):
    ...
    with tf.GradientTape(watch_accessed_variables=False) as tape:
      ...
      _, _, ref_log_probs = self.actor(states)
      ref_log_probs = tf.stop_gradient(ref_log_probs)
      q1, q2 = self.critic(states, actions)
      q = tf.minimum(q1, q2)
      actor_loss = tf.reduce_sum(is_non_absorbing_mask *
                                 (self.alpha * tf.exp(log_probs) * 
                                  (log_probs - ref_log_probs) - q)) / (
